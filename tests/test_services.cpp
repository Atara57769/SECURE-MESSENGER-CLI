...
protected:
    ...
    void SetUp() override {
        ...
        repo = std::make_unique<repository::Repository>(*db);
        repo->create_tables();
...
TEST_F(ServicesTest, RegisterUserSuccessAndFailure) {
    ...
    try {
        services::register_user(short_pass, *repo);
        FAIL() << "Should have thrown ServiceException";
    } catch (const services::ServiceException& e) {
        EXPECT_EQ(e.status_code(), 400);
    }
...
TEST_F(ServicesTest, AuthenticateUserSuccessAndFailure) {
    ...
    services::register_user(reg_req, *repo);

    // 1. Success login
    services::LoginRequest login_req{"alice", "secret123"};
    nlohmann::json login_res = services::authenticate_user(login_req, *repo);
    ...
    try {
        services::authenticate_user(bad_pass, *repo);
        FAIL() << "Should have thrown ServiceException";
    } catch (const services::ServiceException& e) {
    ...
    try {
        services::authenticate_user(bad_user, *repo);
        FAIL() << "Should have thrown ServiceException";
    } catch (const services::ServiceException& e) {
        EXPECT_EQ(e.status_code(), 401);
    }
...
TEST_F(ServicesTest, SendAndFetchMessages) {
    // Register users
    services::register_user({"alice", "secret123"}, *repo);
    services::register_user({"bob", "secret456"}, *repo);

    // 1. Send message successfully
    services::SendMessageRequest send_req{"hello bob!", {"bob"}};
    auto send_res = services::process_send_message(send_req, "alice", *repo);
    ...
    // 2. Fetch messages
    auto alice_msgs = services::fetch_messages("alice", *repo);
    ASSERT_EQ(alice_msgs.size(), 1);
    EXPECT_EQ(alice_msgs[0].content, "hello bob!");

    auto bob_msgs = services::fetch_messages("bob", *repo);
    ...
    EXPECT_THROW({
        services::process_send_message(send_bad_req, "alice", *repo);
    }, services::ServiceException);
}
...
TEST_F(ServicesTest, EditMessage) {
    ...
    auto send_res = services::process_send_message({"hello bob", {"bob"}}, "alice", *repo);
    int msg_id = send_res[0].id;

    // 1. Edit own message successfully
    services::UpdateMessageRequest edit_req{"hello bob (edited)"};
    auto edit_res = services::edit_message(msg_id, "alice", edit_req, *repo);
    ...
    try {
        services::edit_message(msg_id, "bob", edit_req, *repo);
        FAIL() << "Should have thrown ServiceException";
    } catch (const services::ServiceException& e) {
...
TEST_F(ServicesTest, DeleteMessage) {
    ...
    try {
        services::delete_message(msg_id, "bob", *repo);
        FAIL() << "Should have thrown ServiceException";
    } catch (const services::ServiceException& e) {
        EXPECT_EQ(e.status_code(), 403);
    }
    ...
    auto delete_res = services::delete_message(msg_id, "alice", *repo);
    ...
    try {
        services::edit_message(msg_id, "alice", {"new text"}, *repo);
        FAIL() << "Should have thrown ServiceException";
    } catch (const services::ServiceException& e) {
        EXPECT_EQ(e.status_code(), 400);
    }
...
TEST_F(ServicesTest, ValidateStreamToken) {
    ...
    auto login_res2 = services::authenticate_user({"alice", "secret123"}, *repo);
    std::string token2 = login_res2["access_token"].get<std::string>();

    // Token1 validation should now fail
    EXPECT_THROW({
        services::validate_stream_token(token1, *repo);
    }, services::ServiceException);

    // Token2 should succeed
    auto val_res2 = services::validate_stream_token(token2, *repo);
