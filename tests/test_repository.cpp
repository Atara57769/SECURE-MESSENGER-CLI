...
protected:
    // We use a unique temporary filename or memory database.
    // Since some tests might run concurrently or need isolation, an in-memory db is ideal.
...
TEST_F(RepositoryTest, UserOperations) {
    ...
    auto user_opt = repo->get_user_by_username("alice");
    EXPECT_FALSE(user_opt.has_value());

    // 2. Create user
    repository::User alice = repo->create_user("alice", "hashed_password_123");
    ...
    // 3. Retrieve user
    user_opt = repo->get_user_by_username("alice");
    ...
    repo->increment_login_version(alice.id);
    user_opt = repo->get_user_by_username("alice");
    ASSERT_TRUE(user_opt.has_value());
...
TEST_F(RepositoryTest, MessageOperations) {
    ...
    repository::Message msg = repo->create_message(sender, recipient, ciphertext);
    EXPECT_GT(msg.id, 0);
    ...
    auto msg_opt = repo->get_message_by_id(msg.id);
    ...
    auto alice_msgs = repo->get_messages_for_user("alice");
    ASSERT_EQ(alice_msgs.size(), 1);
    EXPECT_EQ(alice_msgs[0].id, msg.id);

    auto bob_msgs = repo->get_messages_for_user("bob");
    ASSERT_EQ(bob_msgs.size(), 1);
    EXPECT_EQ(bob_msgs[0].id, msg.id);

    auto charlie_msgs = repo->get_messages_for_user("charlie");
    EXPECT_TRUE(charlie_msgs.empty());

    // 4. Update message content
    std::string new_ciphertext = "encrypted_hello_world_v2";
    repo->update_message(msg.id, new_ciphertext, false);

    msg_opt = repo->get_message_by_id(msg.id);
    ASSERT_TRUE(msg_opt.has_value());
    ...
    repo->update_message(msg.id, new_ciphertext, true);
    ...
    alice_msgs = repo->get_messages_for_user("alice");
